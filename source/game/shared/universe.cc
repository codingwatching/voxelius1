...
bool universe::load_chunk(const ChunkCoord &cpos)
{
    if(auto chunk = world::find(cpos)) {
        if(chunk->type == ChunkType::Inhabited)
            return true;
        return false;
    }
    ...
    if(fstools::read_bytes(path, buffer)) {
        auto chunk = Chunk::create(ChunkType::Inhabited);
